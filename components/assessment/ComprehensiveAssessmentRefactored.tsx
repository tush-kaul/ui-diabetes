...
import { 
	Eye, Heart, Brain, Activity, Zap, Footprints, 
	Stethoscope, LeafIcon as Liver, TestTube, UserCheck, 
	Save, Plus, X 
} from "lucide-react";
import { 
...
import { getRiskColor, getStatusColor } from "./utils";

export default function ComprehensiveAssessmentRefactored({
	initialSubTab,
	onNavigate,
}: AssessmentProps) {
	const [activeTab, setActiveTab] = useState(initialSubTab || "complications");
	...
	const [customFollowUp, setCustomFollowUp] = useState<Record<string, string>>({});
	...
	const assessmentItems: AssessmentItem[] = [
		{
			...
			icon: Eye,
			status: "Assessed",
			actualDiagnosis: "Moderate NPDR with microaneurysms and hard exudates",
			lastAssessed: "15/10/2023",
			nextDue: "15/04/2024",
			riskLevel: "moderate",
			investigations: ["Fundus Photography", "OCT", "Fluorescein Angiography"],
			consultations: ["Ophthalmology"],
			actionOptions: ["continue same", "optimize medications", "add medications", "refer to specialist", "order investigations"],
		...
		},
		{
			...
			icon: Activity,
			status: "Assessed",
			actualDiagnosis: "Normal kidney function - eGFR 85 ml/min/1.73m², ACR 15 mg/g",
			lastAssessed: "01/07/2024",
			nextDue: "01/07/2025",
			riskLevel: "low",
			investigations: ["Serum Creatinine", "Urine Routine", "Urine ACR", "Urine PCR", "Potassium", "Sodium", "Chloride", "Ultrasound Abdomen", "Calcium", "Phosphorus", "Uric Acid", "Vitamin D", "PTH", "FBS", "PPBS", "HbA1c"],
			consultations: ["Ophthalmologist", "Neurologist", "Cardiologist", "Nutritionist", "Others"],
			actionOptions: ["continue same", "optimize medications", "order investigations"],
			selectedAction: "continue same",
			personalNotes: "",
			hasDrawing: false,
		...
		},
		{
			...
			icon: Zap,
			status: "Assessed",
			actualDiagnosis: "Asymptomatic - Normal monofilament test, intact vibration sense",
			lastAssessed: "15/10/2023",
			nextDue: "15/10/2024",
			riskLevel: "low",
		...
		},
		{
			...
			icon: Heart,
			status: "Assessed",
			actualDiagnosis: "Mild ECG changes - T wave flattening in V5-V6, ECHO EF 60%",
			lastAssessed: "15/10/2023",
			nextDue: "15/04/2024",
			riskLevel: "moderate",
		...
		},
		{
			...
			actionOptions: ["order investigations", "refer to specialist", "continue same"],
		...
		},
		{
			...
			nextDue: "Due now",
			riskLevel: "unknown",
			investigations: ["Liver Function Tests", "Ultrasound Abdomen", "FibroScan", "FIB-4 Score"],
		...
		},
		{
			...
			icon: Footprints,
			status: "Assessed",
			actualDiagnosis: "Low risk - Intact sensation, good circulation, no deformities",
			lastAssessed: "15/10/2023",
			nextDue: "15/10/2024",
			riskLevel: "low",
			investigations: ["Foot Examination", "Doppler Studies", "X-ray Foot"],
			consultations: ["Podiatry", "Vascular Surgery"],
			actionOptions: ["continue same", "refer to specialist", "order investigations"],
			selectedAction: "continue same",
			personalNotes: "",
			hasDrawing: false,
			hasContinuousData: false,
		...
		<div className="p-8 bg-gradient-to-br from-gray-50 to-gray-100">
			...
			<Tabs
				...
				<TabsList className="mb-4 overflow-x-auto whitespace-nowrap">
					...
					<TabsTrigger value="cardiac">Cardiac Assessment</TabsTrigger>
					...
					<TabsTrigger value="orders">Order Tests/Investigations</TabsTrigger>
								...
								<Card
									...
									<CardContent className="space-y-4">
										...
										<div className="bg-gray-50 p-3 rounded-lg">
											...
											<p className="text-sm mt-1">{item.actualDiagnosis}</p>
											<div className="flex justify-between text-xs text-gray-500 mt-2">
												<span>Last: {item.lastAssessed}</span>
										...
										<div>
											...
											<Select
												value={actionPlans[item.name] || item.selectedAction}
												onValueChange={(value) =>
													handleActionChange(item.name, value)
												}>
										...
										<div className="flex space-x-2">
											<Button
												...
												onClick={() => {
													item.investigations.forEach((inv) =>
														addToOrders(inv)
													);
												}}>
											...
											<Button
												...
												className="flex-1 bg-transparent"
												onClick={() => {
													item.consultations.forEach((cons) =>
														addToReferrals(cons)
													);
												}}>
						...
						<CardContent className="space-y-4">
							<div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
								<p className="text-sm text-blue-800 mb-3">
									Pending orders from assessments will appear here
								</p>
						...
						<CardContent className="space-y-4">
							<div className="bg-green-50 border border-green-200 rounded-lg p-4">
								<p className="text-sm text-green-800 mb-3">
									Pending referrals from assessments will appear here
								</p>
				...
				{/* Placeholder tabs for other assessments */}
				<TabsContent value="cardiac">
					<Card className="bg-white shadow-lg">
						...
						<CardContent>
							<p className="text-gray-500">
								Cardiac assessment component would go here. This can be extracted 
								similarly to the other components.
							</p>
						</CardContent>
					</Card>
				</TabsContent>
				...
				<TabsContent value="neuro">
					<Card className="bg-white shadow-lg">
						...
						<CardContent>
							<p className="text-gray-500">
								Neurological assessment component would go here.
							</p>
						</CardContent>
					</Card>
				</TabsContent>
				...
				<TabsContent value="liver">
					<Card className="bg-white shadow-lg">
						...
						<CardContent>
							<p className="text-gray-500">
								Liver assessment component would go here.
							</p>
						</CardContent>
					</Card>
				</TabsContent>
				...
				<TabsContent value="foot">
					<Card className="bg-white shadow-lg">
						...
						<CardContent>
							<p className="text-gray-500">
								Diabetic foot assessment component would go here.
							</p>
						</CardContent>
					</Card>
				</TabsContent>
